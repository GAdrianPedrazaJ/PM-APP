          ...
          <label> Concepto (Labores)
            ...
            <ul class="labores-list">
              <li v-for="(lab, i) in form.labores" :key="i">
                {{ lab }} <button type="button" class="btn btn-ghost" @click="removeLabor(i)">✕</button>
              </li>
            </ul>
          </label>
...
export default {
  ...
  setup() {
    ...
    // API base (usa variable de entorno VITE_API_BASE o rutas relativas)
    const API_BASE = import.meta.env.VITE_API_BASE || '';
    const CASINO_API = `${API_BASE}/api/casino`;

    // Generador de id local (si la API no devuelve id)
    const generateId = () => 'ca_' + Math.random().toString(36).slice(2, 9);
    const assignId = (r) => ({ ...r, _id: r._id || r.id || generateId() });
    ...
    // MOD: carga inicial desde API; si falla, usa localStorage
    onMounted(async () => {
      try {
        await fetchAll();
      } catch (e) {
    ...
    // MOD: funciones que consumen la API REST
    async function fetchAll() {
      const res = await fetch(CASINO_API);
      if (!res.ok) throw new Error('Error fetching casinos');
      const data = await res.json();
      rows.value = Array.isArray(data) ? data : (data.casino || data.casinos || []);
      rows.value = rows.value.map(r => assignId(r));
      persist();
    }
    ...
    async function createRow(payload) {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Error creating casino: ${res.status} ${text}`);
      }
      return res.json();
    }

    async function updateRow(id, payload) {
      const res = await fetch(`${CASINO_API}/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`Error updating casino: ${res.status} ${text}`);
      }
      return res.json();
    }

    async function deleteRowApi(id) {
      const res = await fetch(`${CASINO_API}/${id}`, { method: 'DELETE' });
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`Error deleting casino: ${res.status} ${text}`);
      }
      return res.json();
    }
    ...
    // MOD: save usa API y luego refresca lista; si falla, aplica fallback local
    const save = async () => {
      ...
      try {
        if (editing.value && (form.value._id || form.value.id)) {
          const id = form.value._id || form.value.id;
          await updateRow(id, {
            centro: form.value.centro,
            nombreCuenta: form.value.nombreCuenta,
            numeroCuenta: form.value.numeroCuenta,
            labores: form.value.labores,
        ...
        console.error(err);
        // Fallback local: mantener experiencia si API falla
    ...
    // MOD: delete usa API y luego refresca lista; fallback local si falla
    const deleteRow = async () => {
      if (!toDelete.value) return;
      try {
        const id = toDelete.value._id || toDelete.value.id;
    ...
    return {
      q, rowsComputed, estadoClass, panelOpen, openCreate, openEdit, closePanel,
      form, save, editing, confirmDelete, deleteConfirmOpen, deleteRow, toDelete,
...
.panel { background: #fff; width: 100%; max-width: 480px; height: 100%; padding: 16px; box-shadow: -4px 0 20px rgba(0,0,0,0.2); border-left: 1px solid #e6f2ea; display: flex; flex-direction: column; animation: slideIn 0.3s ease-out; }
...
.labores-input { display: flex; gap: 8px; margin-top: 6px; }
